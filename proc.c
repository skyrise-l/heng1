...
PRIVATE int Random();
PRIVATE int Ring0_schedule();
PRIVATE int Ring3_schedule();
/*****************************************************************************
 *                                schedule
 *****************************************************************************/
/**
...
PUBLIC void schedule()
{
	...
	int random = Random();
	//disp_str("I am in debug\n");
	//disp_int(random);
	int ret;
	if (p_proc_ready->ring != 0)
	{
		out_queue();
	}
	if (random > 0)
...
{
	struct proc *p;
	int greatest_ticks = 0, allBlock = 0;
...
PRIVATE int out_queue()
{
	...
	if (LastPriority == 1)
	{
		task_queue2.proc_pid[task_queue2.start] = proc2pid(p_proc_ready);
		task_queue2.start = (task_queue2.start + 1) % MAX_Queue;
		task_queue1.end = (task_queue1.end + 1) % MAX_Queue;
		p_proc_ready->useTime = 0;
	}
	else if (LastPriority == 2)
	{
		task_queue3.proc_pid[task_queue3.start] = proc2pid(p_proc_ready);
		task_queue3.start = (task_queue3.start + 1) % MAX_Queue;
		task_queue2.end = (task_queue2.end + 1) % MAX_Queue;
		p_proc_ready->useTime = 0;
	}
	else if (LastPriority == 3)
	{
		task_queue3.proc_pid[task_queue3.start] = proc2pid(p_proc_ready);
		task_queue3.start = (task_queue3.start + 1) % MAX_Queue;
		task_queue3.end = (task_queue3.end + 1) % MAX_Queue;
		p_proc_ready->useTime = 0;
	}
	return 0;
}
...
PRIVATE int Ring3_schedule()
{
	int i, right = 0, queueNum = 0, whileNum = 0;
	struct proc *p;

	while (task_queue1.start != task_queue1.end)
	{
		...
		else
		{
			...
			{
				task_queue2.proc_pid[task_queue2.start % MAX_Queue] = task_queue1.proc_pid[task_queue1.end];
				task_queue2.start = (task_queue2.start + 1) % MAX_Queue;
				task_queue1.end = (task_queue1.end + 1) % MAX_Queue;
			}
		}
	}
	while (right != 1 && task_queue2.start != task_queue2.end)
	{
		p = &FIRST_PROC + task_queue2.proc_pid[task_queue2.end];
		if (p->p_flags == FREE_SLOT)
		{
			task_queue2.end = (task_queue2.end + 1) % MAX_Queue;
		}
		else
		{
			if (p->p_flags == 0)
			{
				if (p->ticks == 0)
				{
					p->ticks = task_queue2.give_ticks;
				}
				right = 1;
				LastPriority = 2;
				break;
			...
			{
				task_queue3.proc_pid[task_queue3.start] = task_queue2.proc_pid[task_queue2.end];
				task_queue3.start = (task_queue3.start + 1) % MAX_Queue;
				task_queue2.end = (task_queue2.end + 1) % MAX_Queue;
			}
		}
	}
	queueNum = task_queue3.end - task_queue3.start;
	while (right != 1 && task_queue3.start != task_queue3.end)
	{
		p = &FIRST_PROC + task_queue3.proc_pid[task_queue3.end];
		if (p->p_flags == FREE_SLOT)
		{
			task_queue3.end = (task_queue3.end + 1) % MAX_Queue;
		}
		else
		{
			if (p->p_flags == 0)
			{
				if (p->ticks == 0)
				{
					p->ticks = task_queue3.give_ticks;
				}
				right = 1;
				LastPriority = 3;
				break;
			...
			{
				task_queue3.proc_pid[task_queue3.start] = task_queue3.proc_pid[task_queue3.end];
				task_queue3.start = (task_queue3.start + 1) % MAX_Queue;
				task_queue3.end = (task_queue3.end + 1) % MAX_Queue;
			}
		}
		whileNum++;
		if (whileNum == queueNum)
		{
			break; //防止死循环
		}
	}
	...
	/*int flag = 0;
		for (p = proc_table; p < proc_table + NR_TASKS - 1; p++)
		{
			if (p->ticks != -1)
			{
				...
				if (p->startTime == 0)
				{
					task_queue1.proc_pid[task_queue1.start++ % MAX_Queue] = p->pid;
					p->ticks = task_queue1.give_ticks;
				}
				flag = 1;
			}
